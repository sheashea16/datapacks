...
# store the position of forward locator
...
execute store result score @s motion_x2 run data get entity @n[type=interaction,limit=1,tag=forward] Pos[0] 1000
execute store result score @s motion_z2 run data get entity @n[type=interaction,limit=1,tag=forward] Pos[2] 1000

# store the result
execute store result entity @s Motion[0] double 0.005 \
    run scoreboard players operation @s motion_x2 -= @s motion_x1
execute store result entity @s Motion[2] double 0.005 \
